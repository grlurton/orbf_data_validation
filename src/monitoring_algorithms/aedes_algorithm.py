...
bm_zones = ['OUIDAH-KPOMASSE-TORI-BOSSITO', 'BANIKOARA', 'LOKOSSA-ATHIEME',
            'ADJOHOUN-BONOU-DANGBO', 'KOUANDE-OUASSA-PEHUNCO-KEROU',
            'COVE-ZANGNANADO-OUINHI', 'PORTONOVO-AGUEGUES-SEME-PODJI',
...
def classify_facilities(ecart_moyen_pond):
    q4_rev = ecart_moyen_pond['Montant'].quantile(0.4)
    ecart_moyen_pond['Class'] = 'red'
...
def make_facilities_classification(data, ponderation, perc_risk):
    data['claimed_payment'] = list(
                        data.indicator_claimed_value * data['indicator_tarif']
                        )
    data['verified_payment'] = list(
                        data.indicator_verified_value * data['indicator_tarif']
                        )
    ...
    if sum(data['claimed_payment'] - data['verified_payment']) != 0:
        ...
        indicateurs_critiques = list(
            table_1[table_1['% Cumulé'] <= perc_risk].index
        )

        data_classif = data.loc[indicateurs_critiques]
        data_classif = data_classif.reset_index()

        grouped_entity = data_classif.groupby(['entity_name'])
        ecart_moyen_pondere = grouped_entity.apply(get_ecart_pondere,
...
def screen_function(data, mois, **kwargs):
    ...
    try:
        grouped_region = data_classif.groupby(level=1)
        ecart_moyen_pondere = grouped_region.apply(get_ecart_pondere,
                                                   ponderation=ponderation)
        ecart_moyen_pondere = classify_facilities(ecart_moyen_pondere)
    except KeyError:
...
def draw_supervision_months(desc_parameters, **kwargs):
    green_fac = desc_parameters[desc_parameters['Class'] == 'green']
    orange_fac = desc_parameters[desc_parameters['Class'] == 'orange']

    green_sample = list(green_fac.sample(frac=0.2).index)
    orange_sample = list(orange_fac.sample(frac=0.8).index)
    red_sample = list(desc_parameters[desc_parameters['Class'] == 'red'].index)

    return green_sample + orange_sample + red_sample


def simulate_aedes(screening_method, trigger_supervisions,
                   return_parameters, data, dates, **kwargs):
    for date in dates:
        month = date.month
        if month in [1, 7]:
...
def make_cadran(ecart_moyen_pondere):
    ...
    plt.fill_between(np.array([0, q4_rev]),
                     np.array([max_em, max_em]),
                     facecolor='orange')
    plt.fill_between(np.array([q4_rev, max_rev]),
                     np.array([max_em, max_em]),
                     facecolor='red')
    plt.fill_between(np.array([0, q4_rev]),
                     np.array([min_em, min_em]),
                     np.array([.1, .1]),
                     facecolor='green')
    plt.fill_between(np.array([q4_rev, max_rev]),
                     np.array([min_em, min_em]),
                     np.array([.1, .1]), facecolor='orange')
    plt.plot(list(ecart_moyen_pondere['Montant']),
             list(ecart_moyen_pondere['Ecart Moyen Pondéré']), 'ok')
