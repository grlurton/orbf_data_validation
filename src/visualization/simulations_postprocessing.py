...
os.chdir('c://users/grlurton/documents/orbf_data_quality')
...
# TODO this script should be standalone to get all simulated algorithms
# and return a dataframe with all plotting data
...
def ppv(report_description):
    denom = max(sum(report_description.trigger == True), 1)
    ppv = sum((report_description.ooc == True) & (report_description.trigger == True)) / denom
    return ppv
...
# Aggregate at Month Level
    ...
    data_out['ppv'] = reports_payment.groupby(level=[0, 3]).apply(ppv)
    return data_out
